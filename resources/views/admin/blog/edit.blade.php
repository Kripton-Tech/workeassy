...
<div class="container-fluid">
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('admin.blog.update') }}" name="form" id="form" method="post" enctype="multipart/form-data">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="row">
                            <div class="form-group col-sm-12">
                                ...
                                <input type="text" name="heading" id="heading" class="form-control" placeholder="Plese enter heading" value="{{ @old('heading', $data->heading) }}">
                                ...
                                <select name="category_id" id="category_id" class="form-control">
                                    ...
                                    @if($categories->isNotEmpty())
                                        @foreach($categories as $row)
                                            <option value="{{ $row->id }}" @if(@old('category_id', $data->category_id) == $row->id) selected @endif>{{ $row->name }}</option>
                                        @endforeach
                                    @endif
                                </select>
                            ...
                            <div class="form-group col-sm-12">
                                ...
                                <textarea name="body" id="body" class="form-control" placeholder="Plese enter body">{{ @old('body', $data->body) }}</textarea>
                                <span class="kt-form__help error body"></span>
                            </div>
                            <div class="form-group col-sm-12">
                                <label for="tags">Tags</label>
                                <input type="text" name="tags" id="tags" class="form-control" placeholder="Plese enter tags" value="{{ @old('tags', $data->tags) }}">
                                <span class="kt-form__help error tags"></span>
                            </div>
                            <div class="form-group col-sm-12">
                                <label for="cover_image">Cover Image</label>
                                <input type="file" class="form-control dropify" id="cover_image" name="cover_image" data-default-file="{{ $data->cover_image ??'' }}" data-allowed-file-extensions="jpg png jpeg" data-max-file-size-preview="2M" >
